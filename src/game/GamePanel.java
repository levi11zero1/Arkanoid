...
public class GamePanel extends JPanel implements ActionListener, KeyListener {
    ...
    private GameState toGameState() {
        ...
        // Use the primary ball (first in list) when saving state
        Ball primary = balls.isEmpty() ? new Ball(GameConfig.SCREEN_WIDTH/2, GameConfig.SCREEN_HEIGHT/2) : balls.get(0);
        return new GameState(
            levelManager.getCurrentLevel(),
            primary.getPreciseX(), primary.getPreciseY(),
            primary.getVelocity().getDx(), primary.getVelocity().getDy(),
    ...
    // 3) Khôi phục bóng và thanh đỡ: tái tạo danh sách balls với 1 bóng chính từ save
    balls.clear();
    Ball restored = new Ball((int)Math.round(state.ballX), (int)Math.round(state.ballY));
    restored.setPosition(state.ballX, state.ballY);
    restored.setVelocity(new utils.Velocity(state.ballDx, state.ballDy));
    balls.add(restored);
